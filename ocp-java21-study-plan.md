...
| Day | Task |
|-----|------|
| Mon | Assessment Test (score yourself, note gaps) |
| Tue | Ch. 1: Building Blocks — JVM mechanics, primitives, `var`, text blocks |
| Wed | Ch. 2: Operators — ternary, bitwise, compound assignment traps |
| Thu | Ch. 3: Making Decisions — `switch` expressions, pattern matching (Java 21) |
| Fri | Review questions for Ch. 1–3, re-read anything you got wrong |
...
| Day | Task |
|-----|------|
| Mon | Ch. 4 (part 1): Strings, StringBuilder, equality traps |
| Tue | Ch. 4 (part 2): Arrays, Math API, Date/Time API |
| Wed | Ch. 5 (part 1): Method design, access modifiers, varargs |
| Thu | Ch. 5 (part 2): Static members, overloading rules, pass-by-value |
| Fri | Review questions Ch. 4–5, focus on Date/Time (commonly tricky) |
...
| Day | Task |
|-----|------|
| Mon | Ch. 6 (part 1): Inheritance, constructor chaining, `super` |
| Tue | Ch. 6 (part 2): Abstract classes, immutable objects, initialization order |
| Wed | Ch. 7 (part 1): Interfaces (default/static/private methods), enums |
| Thu | Ch. 7 (part 2): **Sealed classes**, **records**, nested classes — Java 21 key topics |
| Fri | Review questions Ch. 6–7, draw inheritance diagrams by hand |
...
| Day | Task |
|-----|------|
| Mon | Ch. 8 (part 1): Lambda syntax, functional interfaces, method references |
| Tue | Ch. 8 (part 2): Built-in functional interfaces (`Predicate`, `Function`, `Supplier`, etc.) |
| Wed | Ch. 9 (part 1): Collections API — `List`, `Set`, `Queue`, `Deque`, `Map` |
| Thu | Ch. 9 (part 2): Generics, bounded wildcards (`? extends`, `? super`), `Comparable`/`Comparator` |
| Fri | Review questions Ch. 8–9 — write actual code for any lambdas you got wrong |
...
| Day | Task |
|-----|------|
| Mon | Ch. 10 (part 1): Stream pipeline, lazy evaluation, terminal vs intermediate ops |
| Tue | Ch. 10 (part 2): Collectors, primitive streams, `Optional` |
| Wed | Ch. 11: Exceptions (checked/unchecked), try-with-resources, localization/i18n |
| Thu | Ch. 12 (part 1): Module system basics — `module-info.java`, `requires`, `exports` |
| Fri | Ch. 12 (part 2): Services, open modules, `jlink`, `jdeps`, migration strategies |
...
| Day | Task |
|-----|------|
| Mon | Ch. 13: Concurrency — `ExecutorService`, atomic classes, `CyclicBarrier`, **virtual threads** (Java 21) |
| Tue | Ch. 14: I/O — `Path`/`Files` NIO.2, serialization, `Console` |
| Wed | Full review of your weakest chapters (check Assessment Test gaps from Week 1) |
| Thu | Full timed practice test (simulate exam: 50 questions, 90 min, no book) |
| Fri | Review wrong answers, read "Exam Essentials" sections in weak chapters only |
